...
| Evaluator / repair | ID 完整且不重复；引文必须来自对应文章；摘要不得新增数字；有限中文限定语不能丢失。反馈包含上一稿与错误码，最多修正一次 |
...
检查器不是第二个 LLM，自检通过不能证明语义完全正确。英文限定语、数字单位关系、同一事件矛盾仍需人工复核。采集阶段没有逐源持久化恢复，重新采集导致输入变化时会生成新检查点。失败或进程中断时未知的服务端 token 消耗无法准确计入；正常响应用量累计保存。
...
2026-09-21 本机真实采集四源成功，生成两家公司各 8 条的原始资讯预览。真实百炼与 SMTP 必须在用户配置凭据后实测；未配置前不能宣称已投递或模型质量已验收。远端 CI 以 GitHub Actions 对应提交结果为准。
